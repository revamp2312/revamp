...
const ExploreCardContainer = () => {
  return (
    <div
     ...
     className="explore-container w-full max-w-[1440px] m-auto flex justify-center items-center py-20 px-6 tablet:px-14 laptop:px-[70px]">
      <div className="explore-wrapper flex flex-col items-center justify-center gap-12">
        ...
        <div className="explore-card-container flex flex-wrap justify-center items-start gap-12">
          {exploredata.map((card) => {
            return <ExploreCard key={card.id} title={card.title} content={card.content} number={card.number} icon={card.icon} backgroundC={card.backgroundC} boxSHADOW={card.boxSHADOW} arrow={card.arrow} deleverText={card.deleverText} whatwedeiver={card.whatwedeiver} right={card.right}/>;
          })}
        </div>
